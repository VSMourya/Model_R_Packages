...
fit_logistic_lasso <- function(data, predictors, response, lambda = 1, top_k = NULL) {
  ...
  lasso_model <- glmnet(x_train, y_train, alpha = 1, family = "binomial", lambda = lambda)

  saveRDS(lasso_model, "lasso_logistic")

  # Predict and calculate accuracy for the training data
  predictions_train <- predict(lasso_model, s = lambda, newx = x_train, type = "response")
  correct_predictions_train <- mean((predictions_train > 0.5) == y_train)

  # Predict and calculate accuracy for the test data
  predictions_test <- predict(lasso_model, s = lambda, newx = x_test, type = "response")
  ...
  print("Lasso Logistic Regression model is saved in the present directory with the name lasso_logistic")
  print("--------------------------------------------------------------------------------------------------------------------------------------------")


  return(list(model = lasso_model, accuracy_train = correct_predictions_train, accuracy_test = correct_predictions_test, predictors = predictors))
}
